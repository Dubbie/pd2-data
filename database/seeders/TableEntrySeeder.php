...
class TableEntrySeeder extends Seeder
{
    /**
    ...
    private function createEntries(array $entries): void
    {
        foreach ($entries as $value) {
            try {
                TableEntry::updateOrCreate(['key' => $value['key']], ['value' => $value['value']]);

                echo "Created / Updated: " . $value['key'] . PHP_EOL;
            } catch (\Exception $e) {
                echo $e->getMessage() . PHP_EOL;

                TableEntry::updateOrCreate(['key' => $value['key']], ['value' => 'FIX: ' . $value['key']]);
            }
        }
    }
}
